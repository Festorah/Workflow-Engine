...
class AbstractTask(models.Model):
    ...
    if you need to assign a task, you can add a node which gets the task done
    """

    flow_task = fields.TaskReferenceField("Task")
    flow_task_type = models.CharField("flow_task_type", max_length=31)
    status = models.CharField("status", choices=STATUS_CHOICE.choices, max_length=15)
    create_datetime = models.DateTimeField(auto_now_add=True)
    update_datetime = models.DateTimeField(auto_now=True)
    finish_datetime = models.DateTimeField(blank=True, null=True)
    ...
    previous = models.ManyToManyField(
        "self", symmetrical=False, related_name="leading", verbose_name="previous"
    )
