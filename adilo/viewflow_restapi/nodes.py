...
from . import activations, edges, mixins
...
class NextNodeMixin(object):
    def _outgoing(self):
        if self._next:
            return [edges.Edge(src=self, dst=self._next, edge_class="next")]
        return []
...
class Start(mixins.PermissionMixin, NextNodeMixin, Node, ViewArgsMixin):
    ...
    @property
    def view(self):
        log.debug("Generate view based on parameters")
        return self._view(**self._view_args)

    def urls(self):
        log.debug("Back to Start.urls")
        urls = super().urls()
...
class View(mixins.PermissionMixin, NextNodeMixin, Node, ViewArgsMixin):
    ...
    def urls(self):
        ...
        log.debug(f"Back to View.urls: {urls}")
        return urls
